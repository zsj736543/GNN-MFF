...
class DataProcessor:
    ...
    def normalize(self, nx_graph):
        ''' 
            Normalization is find_hw_project_foldersa step to replace the label of a node to a value -> replace all the variable name and numeric values with a high-level type.
        '''
...
class HW2GRAPH:
    ...
    def flatten(self, input_path, flattened_hw_path):
        ...
        for verilog_file in glob(fr'{input_path}/*.v'):
            ...
            with open(verilog_file, 'r', encoding='utf-8', errors='ignore') as infile:
